 ...
 * @returns 是否相等
 */
export const floatEqual = (a, b) => Math.abs(a - b) < 1e-5;
/** ******点、线相关**************************************************** */
/**
 * 计算两个点的中点
 *
 ...
 * @returns 斜率数值
 */
export const lineSlope = (p1, p2) => Math.abs(p2.x - p1.x) === 0 ? Infinity : (p2.y - p1.y) / (p2.x - p1.x);
/**
 * 弧度转化为角度
 *
 * @param radian - 弧度值
 * @returns 角度值
 */
export const radianToAngle = (radian) => (180 * radian) / Math.PI;
/**
 * 弧度转化为角度
 *
 * @param angle - 弧度值
 ...
 * @returns 直线倾角
 */
export const lineRadian = (start, end, angle) => {
 ...
 * @returns 直线倾角
 */
export const lineAngle = (start, end, angle) => radianToAngle(lineRadian(start, end, angle));
/**
 * 计算两点之间的距离
 *
 * @param p1 - 线端点的坐标
 ...
 * @returns 旋转后的点坐标
 */
export const rotate = (point, deg, o = { x: 0, y: 0 }) => {
 ...
 * @returns 起始点坐标
 */
export const lineToStartEndPoint = (line) => {
 ...
 * @returns 起始点坐标
 */
export const startEndPointToLine = (start, end) => ({
    x1: start.x,
    y1: start.y,
    x2: end.x,
    y2: end.y
 ...
 */
export const pointOnLine = (point, line) => {
 ...
 * @returns 在直线上返回0，在直线下方返回-1，上方返回1
 */
export const pointOnLineByEquation = (point, equation) => {
 ...
 * @returns 垂涎方程
 */
export const verticalLineEquation = (line, point) => {
    const { start, end } = lineToStartEndPoint(line);
    const k1 = lineSlope(start, end);
    const k = -1 / k1;
    const b = point.y - k * point.x;
    const verticalLine = { k, b };
    if (Math.abs(k) === Infinity) {
 ...
 */
export const lineCrossPointByEquation = (l1, l2) => {
    const { k: k1, b: b1 } = l1;
    const { k: k2, b: b2 } = l2;
    // 处理垂直于x轴的情况
 ...
 * @returns 垂点坐标
 */
export const verticalLinePoint = (line, point) => {
    const { start, end } = lineToStartEndPoint(line);
 ...
 */
export const verticalBisectorEquation = (start, end) => {
    const center = midpoint(start, end);
    const k1 = lineSlope(start, end);
    const k = k1 === Infinity ? 0 : -1 / k1;
    const b = center.y - k * center.x;
    return { k, b, x: k1 === Infinity ? start.x : NaN };
};
/**
 * 求两直线交点
 *
 ...
 * @returns 两直线交点坐标，平行线返回null
 */
export const lineCrossPoint = (l1, l2) => {
 ...
 */
export const lineSegmentCrossPoint = (l1, l2) => {
    // 判断是否有一个线段的两个端点都在另一个线段的同一边
    if (pointOnLine({ x: l1.x1, y: l1.y1 }, l2) *
 ...
 * @returns 两线段交点坐标，平行线或线段不相交返回null
 */
export const disPointOnLine = (line, dis, inside = true) => {
    const { start, end } = lineToStartEndPoint(line);
 ...
 * @returns 两线段交点坐标，平行线或线段不相交返回null
 */
export const disPointOnLineByEquation = (equation, point, dis, inside = true) => {
    const { k } = equation;
 ...
 * @returns 点
 */
export const disPointFromLine = (start, end, dis, onTop = true) => {
    const k1 = lineSlope(start, end);
    const k = k1 === Infinity ? 0 : -1 / k1;
    const sita = Math.atan(k);
 ...
 * @returns a、b、c三个角度
 */
export const angleOfTriangle = (A, B, C) => {
    const angle = (radian) => (180 * radian) / Math.PI;
 ...
 * @returns 距离
 */
export const disOfPointToLine = (point, line) => {
    const { start, end } = lineToStartEndPoint(line);
 ...
 * @returns 两个点的坐标
 */
export const angleEquidistantPoints = (A, B, C, d = 15) => {
 ...
 * @returns 目标点的坐标
 */
export const angleBisectorPoint = (A, B, C, d = 30) => {
    // 先求角度等距点
...
// 判断弧线是否要翻转
export const isArcReverse = (A, B, C, d = 15) => {
    ...
    const reverse = (sitaA > sitaC && sitaA - sitaC < Math.PI) ||
 ...
 */
export const angleBisectorEquation = (A, B, C) => {
    ...
    const b = B.y - k * B.x;
    return { k, b, x: k === Infinity ? B.x : NaN };
};
/** ********其他********************************************************* */
/**
 * 射线法判断点是否在多边形内
 ...
 * @returns 是否在多边形内
 */
export const pointInPolygon = (points, p) => {
 ...
 * @returns 在内边距范围内的点
 */
export const handlePadding = (point, padding, width, height) => {
    let { x, y } = point;
 ...
 * @returns
 */
export const getLineSegment = (start, end, dis = 20, angle) => {
 ...
 * @returns
 */
export const getPointsBoundingRect = (points) => {
    let minX = Infinity;
    let maxX = -Infinity;
    let minY = Infinity;
    let maxY = -Infinity;
 ...
 * @returns
 */
export const getControlPoints = ([start, center, end], rate = 0.4) => {
    ...
    if (pointOnLineByEquation(start, equ1) !== pointOnLineByEquation(control1, equ1)) {
        return [{ ...control2, color: "red" }, control1, [equ2]];
    }
    return [{ ...control1, color: "red" }, control2, [equ2]];
};
export const getIncreaseSize = (initMousePos, mousePoint, angle = 0) => {
